...
REG_MAX = 16
bins = tf.range(REG_MAX, dtype=tf.float32)          # <-- module-level
strides = tf.constant([8]*6400 + [16]*1600 + [32]*400, tf.float32)
gx = tf.constant(                       # 0-based cell centres
...
class YOLOv8LiteWrapper:
    def __init__(self, tflite_path,
        ...
        self.box_scale = box_info['quantization_parameters']['scales']  # (N,)
        self.box_zp = box_info['quantization_parameters']['zero_points']  # (N,)
    ...
    def _run(self, inp_np):
        ...
        raw_boxes = (raw_boxes.astype(np.float32) -
                     self.box_zp) * self.box_scale  # per-channel scale
        raw_logits = (raw_logits.astype(np.float32) -
                      self.log_zp) * self.log_scale  # one global scale
